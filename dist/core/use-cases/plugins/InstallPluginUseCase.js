...
const isValidScraperModule = (mod) => {
    if (!mod || typeof mod !== 'object')
        return false;
    const m = mod;
    return (typeof m.sourceName === 'string' &&
...
export const installPlugin = async (scrapperPath, onProgress) => {
    ...
    const savePluginsMetadata = (plugs) => {
    ...
    try {
        ...
        if (alreadyExists) {
            const updatedPlugins = existingPlugins.map(p => p.pluginId === pluginData.pluginId ? finalMetadata : p);
