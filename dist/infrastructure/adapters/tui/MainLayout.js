...
import { buildApplicationListItemLabel, buildListItemLabel, } from '../../../services/list-labels.js';
...
export const MainLayout = ({ autoScan, jobService, applicationService }) => {
    ...
    const [previousPanel, setPreviousPanel] = useState('jobs');
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            try {
                ...
                setStatus(filteredJobs.length === 0
    ...
    const applicationsLabelWidth = Math.max(28, panelContentWidth - Math.max(2, String(applicationsData.length).length) - 2);
    const jobsTotalPages = jobsData.length === 0 ? 0 : Math.ceil(jobsData.length / jobsPageSize);
    const applicationsTotalPages = applicationsData.length === 0 ? 0 : Math.ceil(applicationsData.length / applicationsPageSize);
    const safeJobsPageIndex = jobsTotalPages === 0 ? 0 : Math.min(jobPageIndex, jobsTotalPages - 1);
    const safeApplicationsPageIndex = applicationsTotalPages === 0 ? 0 : Math.min(applicationPageIndex, applicationsTotalPages - 1);
    const jobsPagination = useMemo(() => paginateWithLabels(jobsData, safeJobsPageIndex, jobsPageSize, (job) => buildListItemLabel({
        source: job.source,
        primaryText: job.description,
        secondaryText: job.company,
        availableWidth: jobsLabelWidth,
    }), jobsLabelWidth, 'avisos'), [jobsData, safeJobsPageIndex, jobsPageSize, jobsLabelWidth]);
    const applicationsPagination = useMemo(() => paginateWithLabels(applicationsData, safeApplicationsPageIndex, applicationsPageSize, (application) => buildApplicationListItemLabel({
        source: application.source,
        appliedAt: application.appliedAt,
        primaryText: application.title,
        secondaryText: application.company,
        availableWidth: applicationsLabelWidth,
    }), applicationsLabelWidth, 'postulaciones'), [applicationsData, safeApplicationsPageIndex, applicationsPageSize, applicationsLabelWidth]);
    ...
    const persistKeywordList = async (nextKeywords, nextSelection) => {
    ...
    const handleDeleteKeyword = async () => {
        ...
        const selectedIndex = keywords.findIndex((keyword) => keyword.toLowerCase() === selectedKeyword.toLowerCase());
        if (selectedIndex < 0) {
        ...
        try {
            ...
            zip.forEach((relativePath, zipEntry) => {
                if (!zipEntry.dir) {
                    ...
                    const p = zipEntry.async('nodebuffer').then((content) => {
    ...
    useInput((input, key) => {
        ...
        if (key.pageDown) {
            ...
            if (activePanel === 'applications' && applicationsTotalPages > 0) {
                setApplicationPageIndex((currentPage) => Math.min(applicationsTotalPages - 1, currentPage + 1));
            }
        }
    });
