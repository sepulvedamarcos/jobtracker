// src/core/use-cases/plugins/RunPluginsScanParallelUseCase.ts
...
const saveTempJobs = (pluginId: string, keyword: string, jobs: Job[]) => {
  const tempPath = path.join(
    path.dirname(APP_PATHS.jobs),
    `temp_${pluginId}_${Buffer.from(keyword).toString('base64').slice(0, 10)}.json`
  );
  fs.writeFileSync(tempPath, JSON.stringify(jobs, null, 2));
};

// Leer todos los temp files
const loadAllTempJobs = (): Job[] => {
  const tempDir = path.dirname(APP_PATHS.jobs);
  const entries = fs.readdirSync(tempDir);
  const jobs: Job[] = [];
...
export const runPluginsScanParallel = async (
  plugins: string[],
  ...
  const pluginResults: ParallelScanResult['pluginResults'] = [];
  ...
  const scanPromises = Array.from(loadedPlugins.entries()).map(
    async ([pluginId, scraper], index) => {
      if (signal?.aborted) {
        return [];
      }
  ...
  const seen = new Map<string, Job>(); // URL -> Job (primera aparición)
  let dupesFound = 0;
