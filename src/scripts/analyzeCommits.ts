...
function getCommitType(commitMsg: string): string {
  const msg = commitMsg.toLowerCase();
  if (msg.startsWith('feat') || msg.includes(': add') || msg.includes('feat(')) return 'feat';
  if (msg.startsWith('fix') || msg.includes('fix(')) return 'fix';
  if (msg.startsWith('refactor') || msg.startsWith('perf') || msg.startsWith('revert')) return 'refactor';
  if (msg.startsWith('docs') || msg.startsWith('chore') || msg.startsWith('style') || msg.startsWith('test')) return 'chore';
  return 'other';
}

function parseCommits(previousTag: string) {
...
function runRelease(releaseType: string) {
